						...
						<tbody>
							...
							while ($row = mysqli_fetch_array($sql)) {
								...
								echo "<tr>
										...
										<td>$status</td>
										<td>
											<a href='assign_workload.php?projection_id=$row[projection_id]'
												data-toggle='tooltip' title='Assign Workload'>
												<i class='hgi-stroke hgi-add-square text-success'></i>
											</a>
											<a href='update_projection.php?projection_id=$row[projection_id]'
											   data-toggle='tooltip' title='Update'>
											   <i class='hgi-stroke hgi-pencil-edit-02 text-warning'></i>
											</a>			
											<a href='manage_semester_batch_projection.php?program_code=$program_code&session_id=$session_id&semester_id=$semester_id&batch=$batch&act=del&projection_id=$row[projection_id]'
												data-toggle='tooltip' title='Remove'
												onclick=\"return confirm('Are you sure want to remove projection for lecturer $row[name] for course $row[course_code] Group $row[groupings]?');\">
