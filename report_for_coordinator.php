...
<style>
    /* CSS class to arrange charts vertically */
    .vertical-layout .row {
        display: block;  /* Force rows to stack vertically */
    }
    .vertical-layout .col-lg-6,
    .vertical-layout .col-lg-4, 
    .vertical-layout .col-lg-12 {
        width: 100% !important;  /* Set each column to take full width */
...
<body>
  ...
  ?>
        ...
        <div id="reportSection"> <!-- Start of content to print -->
			...
			<?php 
				...
				if($displayPDFbutton == "yes")
				{
					echo "<div class='text-right mb-3'>
							<button onclick='exportToPDF()' class='btn btn-primary'><i class='mdi mdi-printer'></i> Export to PDF</button>
						</div>";
				}
...
<script>
    function exportToPDF() {
        // Select the report section and temporarily apply the vertical layout class
        const reportElement = document.getElementById('reportSection');
        reportElement.classList.add('vertical-layout');

        // Configure options for html2pdf
        const options = {
            margin:       1,
            filename:     'Chart_Report_<?php echo $program . "_" . $session; ?>.pdf',
        ...
        html2pdf().set(options).from(reportElement).save().then(() => {
            // Remove the vertical layout class to restore original layout
            reportElement.classList.remove('vertical-layout');
        });
    }
</script>
