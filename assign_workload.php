              ...
              <div class="card">
                <div class="card-body">
				  ...
				  <input type="hidden" class="form-control" name="projection_id" value="<?php echo $projection_id; ?>" required />
                    ...
                    <p class="card-description text-primary">
                      <i class="hgi-stroke hgi-work-history"></i> Assign Workload for Lecturer
                    </p>
                      ...
                      <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="lecturer_id" id="lecturer_id" required>
                              ...
                              if ($sqlLect) {
                                  while ($rowLect = mysqli_fetch_assoc($sqlLect)) {
                                      // Check if the lecturer is already assigned to this projection
                                      $selected = ($projection['lecturer_id'] == $rowLect['lecturer_id']) ? 'selected' : '';
                                      echo "<option value='{$rowLect['lecturer_id']}' $selected>{$rowLect['name']} - {$rowLect['specialization']}</option>";
                                  }
                              } else {
                                  echo "<option value=''>Error loading lecturers: " . mysqli_error($conn) . "</option>";
                              }
                              ?>
                            </select>
                        </div>
                    ...
                    <div class="row">
                      <div class="col-md-4">
                        <div class="form-group">
                          ...
                          <input type="number" min="1" class="form-control" name="assigned_hours" value="<?php echo $projection['assigned_hours']; ?>" required />
                        </div>
                      </div>
                    </div>
                    ...
                    <br />
                    <a href="manage_semester_batch_projection.php?program_code=<?php echo urlencode($program_code); ?>&session_id=<?php echo urlencode($session_id); ?>&semester_id=<?php echo urlencode($semester_id); ?>&batch=<?php echo urlencode($batch); ?>" class="btn btn-outline-dark">
